...
class Viewport:
    def __init__(self, canvas, wmin, wmax):
        ...
        self.wmin = wmin
        self.wmax = wmax
        # ângulo da window em graus (0 = sem rotação). Rotação positiva = girar window no sentido anti-horário.
    ...
    def _rotacionar_ponto_em_torno_de(self, x, y, cx, cy, ang_rad):
        """Rotaciona o ponto (x,y) em torno de (cx,cy) por ang_rad (rad)."""
        tx = x - cx
        ty = y - cy
        c = np.cos(ang_rad)
        s = np.sin(ang_rad)
    ...
    def transformar(self, xw, yw, w_adj_min, w_adj_max, vpmin, vpmax):
        """Transforma coordenadas do mundo (xw,yw) para viewport (mantida a função original)."""
    ...
    def desenhar(self, displayfile: DisplayFile):
        ...
        def transform(xw, yw):
            # 1) rotacione o ponto world em torno do centro da window por -window_angle
            xr, yr = self._rotacionar_ponto_em_torno_de(xw, yw, cx, cy, ang_rad)
            # 2) projete o ponto rotacionado no viewport normal (mantendo a função transformar original)
            return self.transformar(xr, yr, w_adj_min, w_adj_max, vpmin, vpmax)
        ...
        for obj in displayfile.listar():
            # para cada vértice: aplicar rotação do mundo (na hora do desenho) e só então transformar para vp
...
class App(tk.Frame):
    ...
    def pan(self, dx, dy):
        """
        dx,dy são deslocamentos na percepção do usuário (p. ex.: clicar em botão '→' quer dizer mover a window para a direita na tela).
        Precisamos converter esse deslocamento para o sistema world levando em conta a rotação da window.
        """
        # converte dx,dy (pixels conceituais do mundo) em vetor rotacionado por -window_angle?
        # Aqui assumimos dx,dy em unidades do mundo (como nas versões anteriores).
        theta = np.deg2rad(self.viewport.window_angle)
        c = np.cos(theta)
        s = np.sin(theta)
        # vetor pedido na orientação da tela -> rotaciona pelo ângulo da window para obter translacao em world
        # Se a window está rotacionada em theta (sens. anti-horário), então para mover a janela visualmente para a direita
        # devemos deslocar os limites do window pelo vetor (dx*cos - dy*sin, dx*sin + dy*cos)
        world_dx = dx * c - dy * s
        world_dy = dx * s + dy * c
