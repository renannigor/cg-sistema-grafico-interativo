...
class Clipping:
    ...
    def clip_curva_para_linhas(self, pontos_controle_total, wmin, wmax):
        ...
        for i in range(num_segmentos):
            inicio_idx = i * 3
            fim_idx = inicio_idx + 4
            pontos_segmento_atual = pontos_controle_total[inicio_idx:fim_idx]
        ...
        for obj in displayfile.listar():
            if obj.tipo == TipoObjeto.CURVA.value:
                ...
                for linha in segmentos_de_linha:
                    p1_vp = transform_coord(linha[0][0], linha[0][1])
                    p2_vp = transform_coord(linha[1][0], linha[1][1])
...
class App(tk.Frame):
    ...
    def abrir_popup_objetos(self):
        ...
        # Aba Curva
        frame_curva = ttk.Frame(notebook)
        notebook.add(frame_curva, text="Curva")
        nome_curva = tk.Entry(frame_curva)
        coords_entry_curva = tk.Entry(frame_curva, width=30)
        tk.Label(frame_curva, text="Nome:").grid(row=0, column=0, sticky="w", pady=2)
        nome_curva.grid(row=0, column=1, sticky="ew")
        tk.Label(frame_curva, text="Pontos de Controle:").grid(row=1, column=0, columnspan=2, sticky="w")
        coords_entry_curva.grid(row=2, column=0, columnspan=2, pady=2, sticky="ew")
        tk.Label(frame_curva, text="Formato: [(x1,y1),(x2,y2),...]").grid(row=3, column=0, columnspan=2, sticky="w")
        tk.Label(frame_curva, text="Mínimo de 4 pontos (depois +3 por segmento)").grid(row=4, column=0, columnspan=2, sticky="w", pady=4)
        tk.Button(frame_curva, text="Adicionar", command=lambda: self.add_obj(nome_curva.get(), TipoObjeto.CURVA.value, list(ast.literal_eval(coords_entry_curva.get())), popup, self.selected_color)).grid(row=5, columnspan=2, pady=10)
