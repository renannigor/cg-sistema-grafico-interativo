...
class GeradorBSpline:
    """Classe para gerar curvas B-Spline usando Forward Differences."""
    # Matriz base da B-Spline, conforme slides
    ...
    def _get_matriz_E(delta: float) -> np.ndarray:
        """Cria a matriz de Forward Differences E para um dado delta."""
        d2 = delta * delta
        d3 = d2 * delta
        return np.array([
            [0,    0,     0,     1],
    ...
    def gerar_segmentos(self, pontos_controle: List[Tuple[float, float]], delta: float = 0.1) -> List[List[Tuple[float, float]]]:
        """Gera todos os segmentos de reta que compõem a curva B-Spline."""
        ...
        # Itera por cada segmento da curva B-Spline
        for i in range(num_curvas):
            # Seleciona os 4 pontos de controle para o segmento atual
            p_controle_segmento = pontos_controle[i : i + 4]
            
            # Constrói os vetores de geometria Gx e Gy
            Gx = np.array([p[0] for p in p_controle_segmento]).reshape(4, 1)
            Gy = np.array([p[1] for p in p_controle_segmento]).reshape(4, 1)

            # Calcula os vetores de coeficientes Cx e Cy
            Cx = self.M_BSPLINE @ Gx
            Cy = self.M_BSPLINE @ Gy
            
            # Calcula as condições iniciais (f, Δf, Δ²f, Δ³f)
            ...
            # Loop para desenhar os pontos da curva (algoritmo fwdDiff)
            for _ in range(num_passos):
                x_velho, y_velho = x, y
                
                # Atualiza as posições e as diferenças
                x += dx
                dx += d2x
                d2x += d3x
...
class Clipping:
    ...
    def clip(self, obj, wmin, wmax, alg_reta):
        ...
        if obj.tipo == TipoObjeto.CURVA.value or obj.tipo == TipoObjeto.B_SPLINE.value:
            return obj
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("SGI 2D - B-Spline e Clipping")
