            ...
            if obj.tipo == TipoObjeto.OBJETO_3D.value:
                ...
                for idx1, idx2 in obj_camera_coords.arestas:
                    ...
                    if tipo_projecao == 'perspectiva':
                        d = self.camera.d
                        if p1_cam.z < 1 and p2_cam.z < 1:
                            continue
                        
                        x1_p = d * p1_cam.x / p1_cam.z if p1_cam.z != 0 else 0
                        y1_p = d * p1_cam.y / p1_cam.z if p1_cam.z != 0 else 0
                        x2_p = d * p2_cam.x / p2_cam.z if p2_cam.z != 0 else 0
                        y2_p = d * p2_cam.y / p2_cam.z if p2_cam.z != 0 else 0
...
class App(tk.Frame):
    def __init__(self, master):
        ...
        self.camera = Camera(vrp=Ponto3D(0, 50, 200), p_foco=Ponto3D(0, 0, 0), vup=(0, 1, 0), d=300)
        ...
        tk.Button(top_row, text="Carregar .obj", command=self.carregar_arquivo_obj).pack(side="left", padx=2)
        ...
        tk.Button(mid_row, text="↓", command=lambda: self.pan(0, 10)).pack(side="left")
        tk.Button(mid_row, text="Zoom +", command=lambda: self.zoom(0.9)).pack(side="left", padx=(5,2))
        tk.Button(mid_row, text="Zoom -", command=lambda: self.zoom(1.1)).pack(side="left")
        ...
        tk.Radiobutton(bottom_row, text="Ortogonal", variable=self.projection_type_var, value="ortogonal", command=self.redesenhar).pack(side="left")
        tk.Radiobutton(bottom_row, text="Perspectiva", variable=self.projection_type_var, value="perspectiva", command=self.redesenhar).pack(side="left")
        
        tk.Label(bottom_row, text="Dist. COP (d):").pack(side="left", padx=(10,0))
        self.dist_slider = tk.Scale(bottom_row, from_=50, to=1000, orient="horizontal", length=200, command=self.atualizar_distancia_projecao)
    ...
    def redesenhar(self):
        self.viewport.desenhar(self.displayfile, self.alg_reta_clip_var.get(), self.projection_type_var.get())
    ...
    def carregar_arquivo_obj(self):
        ...
        try:
            ...
            messagebox.showerror("Erro ao Carregar OBJ", f"Não foi possível ler o arquivo.\n\nErro: {e}")
    ...
    def abrir_popup_objetos(self):
        ...
        tk.Button(frame_obj3d, text="Adicionar", command=lambda: self.add_obj_3d(nome_obj3d.get(), vertices_text.get("1.0", "end-1c"), arestas_text.get("1.0", "end-1c"), popup, self.selected_color)).grid(row=7, columnspan=2, pady=10)
    ...
    def abrir_transformacoes_popup_3d(self, obj):
        ...
        tk.Button(frame_t, text="Aplicar", command=lambda: self.aplicar_transformacao_3d(obj, "translacao", dx=float(dx_t.get() or 0), dy=float(dy_t.get() or 0), dz=float(dz_t.get() or 0), popup=popup)).grid(row=3, columnspan=2)
        ...
        tk.Button(frame_s, text="Aplicar", command=lambda: self.aplicar_transformacao_3d(obj, "escalonamento", sx=float(sx_s.get() or 1), sy=float(sy_s.get() or 1), sz=float(sz_s.get() or 1), popup=popup)).grid(row=3, columnspan=2)
        ...
        tk.Button(frame_r, text="Aplicar", command=lambda: self.aplicar_transformacao_3d(obj, "rotacao", angulo=float(angulo_r.get() or 0), eixo=eixo_var.get(), popup=popup)).grid(row=4, columnspan=2)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("SGI 2D/3D - Projeção em Perspectiva")
