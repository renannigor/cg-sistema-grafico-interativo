    ...
    def desenhar(
        ...
        for obj in displayfile.listar():
            if isinstance(obj, Objeto3D) or isinstance(obj, SuperficieBezier):
                ...
                elif obj.tipo == TipoObjeto.SUPERFICIE_BEZIER.value:
    ...
    def aplicar_transformacao_3d(self, obj, tipo_transf, **kwargs):
        ...
        try:
            if tipo_transf == "translacao":
                ...
                else:  # SuperficieBezier
                    # Aproxima o centro pelos pontos de controle
                    todos_pontos = obj.pontos_controle.flatten()
                    cx = sum(p.x for p in todos_pontos) / 16
                    cy = sum(p.y for p in todos_pontos) / 16
                    cz = sum(p.z for p in todos_pontos) / 16
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("SGI 2D/3D - Superfícies de Bézier")
